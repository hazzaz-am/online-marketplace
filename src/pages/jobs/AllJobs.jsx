...
const AllJobs = () => {
	...
	const pages = count
		? [...Array(Math.ceil(count.count / ITEMS_PER_PAGE)).keys()].map(
				(elem) => elem + 1
		  )
		: [];

	function handlePagination(btnType) {
		if (btnType === "previous" && currentPage !== 1) {
			setCurrentPage(currentPage - 1);
		} else if (btnType === "next" && currentPage !== pages.length) {
			setCurrentPage(currentPage + 1);
		}
	}
